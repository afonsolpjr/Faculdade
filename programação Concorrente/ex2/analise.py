...
parser.add_argument("analise", help="1 para analise de niveis de otimizacao | 2 para analise do tempo de execucao| 3 para analise dos niveis sem criação de graficos", type=int)
parser.add_argument("max_threads", help="numero maximo de threads desejada nas analises",type=int)
args = parser.parse_args()

# Keys:
# ,"flag","n_threads","dimensao",
# "t_init","t_exec","t_fin"
...
with open('dados.csv', newline='') as csvfile:
    ...
    for row in reader:
        ...
        row['t_init'] = float(row['t_init'])
        row['t_exec'] = float(row['t_exec'])
        row['t_fin'] = float(row['t_fin'])
        row['t_total'] = row['t_fin'] + row['t_exec'] + row['t_init']
...
def analise_otimizacao(fazer_grafico):
    ...
    for flag in flags:
        ...
        for dim in dimensoes:
            ...
            flag_execs = [item for item in execucoes if item['flag']==flag and item['dimensao']==dim and item['n_threads'] == 1]
    ...
    if(fazer_grafico):
        plt.savefig('graficos_nprimos/tempos_nivel_otimizacao.png')
        plt.close()
...
def analise_exec(execs):
    ...
    for n_dim in dimensoes:
        for n_t in n_threads:    
            ...
            chave = (n_t,n_dim)
            # print(chave)
            media_init = sum([item['t_init'] for item in exs])/len(exs)
            media_exec = sum([item['t_exec'] for item in exs])/len(exs)
            media_fin = sum([item['t_fin'] for item in exs])/len(exs)
            dados.update({
                chave:{
    ...
    fig.suptitle('Tempos de Execução por Número de Threads (por valor de N)', fontsize=18)
    # fig.subplots_adjust(top=0.93, hspace=0.4)  # espaço vertical extra
    plt.savefig('graficos_nprimos/todos_tempos_execucao.png', dpi=300)
    plt.close(fig)
...
# Uma linha para cada dimensão nn (tamanho do problema)
    ...
    plt.savefig('graficos_nprimos/aceleracao.png')
    plt.close()
    return
...
def graf_eficiencia(dados):
    ...
    plt.savefig('graficos_nprimos/eficiencia.png')
    plt.close()
...
match args.analise:
    ...
    case 2:
        ...
        analise_exec([item for item in execucoes if item['flag']==flag ])
    case 3:
        flag = analise_otimizacao(False)
        print(flag)
