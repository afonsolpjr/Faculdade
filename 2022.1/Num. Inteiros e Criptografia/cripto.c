...
int main(void)
{
	...
	while(opcao!=0)
	{
		...
		"[8] Numeros de Mersenne X Fermat, inserindo n \n"
...
long int qtd_divisores(long int a)
{
	...
	print_vetor(expoentes,distintos[0]);
...
void numeros_perfeitos(long int a)
{
	long int num,soma_div,i;

	num = (potencia(2,(a-1))*(potencia(2,a)-1));
	//    2^s                2^(s-1)-1               na notação do livro.
		...
		if(num*2==soma_div)
		{
			printf("\nOba! O resultado = %ld eh um numero perfeito\n",num);
		}
		else
		{
				printf("\nXii, o resultado = %ld nao eh um numero perfeito\n",num);
		}
	}
